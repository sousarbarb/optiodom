function [RobotEstParam,cost,costSim,numIterations,historyRobotParam] = Rprop_omni4(RobotParam,XGt,Odo,iSamples,options)
  ...
  if strcmp(rproptype,'rprop+')
    while (true)
      ...
      derivParam = zeros(Nsim,(nWh+nL)*2);  % [ dx/dL(1) dx/dL(2) dx/dD(1) dx/dD(2) dx/dD(3) dx/dD(4) dy/dL(1) dy/dL(2) dy/dD(1) dy/dD(2) dy/dD(3) dy/dD(4) ]
      costDeriv  = zeros(1,nWh+nL);         % [ dE/dL(1) dE/dL(2) dE/dD(1) dE/dD(2) dE/dD(3) dE/dD(4) ]
      ...
      for i=1:Nsim
        ...
        for k=2:itf(i)
          % Common variables
          deltaD  = (  D(1)*Odo{i}(k,1) - D(2)*Odo{i}(k,2) + D(3)*Odo{i}(k,3) - D(4)*Odo{i}(k,4) );
          deltaDN = ( -D(1)*Odo{i}(k,1) - D(2)*Odo{i}(k,2) + D(3)*Odo{i}(k,3) + D(4)*Odo{i}(k,4) );
          deltaTH = ( -D(1)*Odo{i}(k,1) - D(2)*Odo{i}(k,2) - D(3)*Odo{i}(k,3) - D(4)*Odo{i}(k,4) ) * a_pi_2L1L2nCe;
          ...
          if ( abs(deltaTH)/0.040 < 1e-5 )
            ...
            derivParam(i,7) = derivParam(i,7) + ( -deltaD * a_cosTh + deltaDN * a_sinTh ) * ( XOdoF(i,3) - XGt{i}(1,3) );
            derivParam(i,9) = derivParam(i,9)       + ...
                              Odo{i}(k,1) * a_sinTh - ...
                              Odo{i}(k,1) * a_cosTh - ...
                              a_dDCosdDnSin * a_Wh{i}(k,1) * 2 / (L(1)+L(2));
            derivParam(i,10)= derivParam(i,10)      - ...
                              Odo{i}(k,2) * a_sinTh - ...
                              Odo{i}(k,2) * a_cosTh - ...
                              a_dDCosdDnSin * a_Wh{i}(k,2) * 2 / (L(1)+L(2));
            derivParam(i,11)= derivParam(i,11)      + ...
                              Odo{i}(k,3) * a_sinTh + ...
                              Odo{i}(k,3) * a_cosTh - ...
                              a_dDCosdDnSin * a_Wh{i}(k,3) * 2 / (L(1)+L(2));
            derivParam(i,12)= derivParam(i,12)      - ...
                              Odo{i}(k,4) * a_sinTh + ...
                              Odo{i}(k,4) * a_cosTh - ...
          ...
          if (k == iSamples{i}(iseg))
            ...
            a_derivParam(4) = derivParam(i,4) * a_pi_4nCe;
            a_derivParam(5) = derivParam(i,5) * a_pi_4nCe;
            a_derivParam(6) = derivParam(i,6) * a_pi_4nCe;
            a_derivParam(7) = derivParam(i,7) * a_pi_4L1L2nCe;
            a_derivParam(8) = a_derivParam(7);
            a_derivParam(9) = derivParam(i,9) * a_pi_4nCe;
            a_derivParam(10)= derivParam(i,10)* a_pi_4nCe;
            a_derivParam(11)= derivParam(i,11)* a_pi_4nCe;
            a_derivParam(12)= derivParam(i,12)* a_pi_4nCe;

            % Computation of the cost derivatives: [ dE/dL dE/dD(1) dE/dD(2) ]
            costDeriv(1) = costDeriv(1) + 2 * ( a_XOdoF(1) - XGt{i}(k,1) ) * a_derivParam(1) + 2 * ( a_XOdoF(2) - XGt{i}(k,2) ) * a_derivParam(7);
            costDeriv(2) = costDeriv(2) + 2 * ( a_XOdoF(1) - XGt{i}(k,1) ) * a_derivParam(2) + 2 * ( a_XOdoF(2) - XGt{i}(k,2) ) * a_derivParam(8);
            costDeriv(3) = costDeriv(3) + 2 * ( a_XOdoF(1) - XGt{i}(k,1) ) * a_derivParam(3) + 2 * ( a_XOdoF(2) - XGt{i}(k,2) ) * a_derivParam(9);
            costDeriv(4) = costDeriv(4) + 2 * ( a_XOdoF(1) - XGt{i}(k,1) ) * a_derivParam(4) + 2 * ( a_XOdoF(2) - XGt{i}(k,2) ) * a_derivParam(10);
            costDeriv(5) = costDeriv(5) + 2 * ( a_XOdoF(1) - XGt{i}(k,1) ) * a_derivParam(5) + 2 * ( a_XOdoF(2) - XGt{i}(k,2) ) * a_derivParam(11);
            costDeriv(6) = costDeriv(6) + 2 * ( a_XOdoF(1) - XGt{i}(k,1) ) * a_derivParam(6) + 2 * ( a_XOdoF(2) - XGt{i}(k,2) ) * a_derivParam(12);

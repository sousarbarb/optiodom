% Suptitle parameters
suptitleParam.use = false;
suptitleParam.useFilename = false;
...
for i=1:length(t)
  ...
  xmin = min([ XGt{i}(:,1) ; XOdo{i}(:,1) ; XOdoCal{i}(:,1) ]);
  xmax = max([ XGt{i}(:,1) ; XOdo{i}(:,1) ; XOdoCal{i}(:,1) ]);
  ymin = min([ XGt{i}(:,2) ; XOdo{i}(:,2) ; XOdoCal{i}(:,2) ]);
  ymax = max([ XGt{i}(:,2) ; XOdo{i}(:,2) ; XOdoCal{i}(:,2) ]);
  ...
  if (~isempty(Method.sampleDist))
    ...
    plot(XOdoCal{i}(iSamples{i} ,1),XOdoCal{i}( iSamples{i} ,2),'x','Color',initialColorOrder(2,:))
    plot(XGt{i}(    iSamples{i} ,1),XGt{i}(     iSamples{i} ,2),'x','Color',initialColorOrder(3,:))
  end
  grid on
  xlim([ xmin xmax ])
  ylim([ ymin ymax ])
...
for i=1:length(t)
  figure
  initialColorOrder = get(gca,'ColorOrder');
  ymin = rad2deg(min(wrapToPi([ XOdo{i}(:,3) ; XOdoCal{i}(:,3) ; XGt{i}(:,3) ])));
  ymax = rad2deg(max(wrapToPi([ XOdo{i}(:,3) ; XOdoCal{i}(:,3) ; XGt{i}(:,3) ])));
  ymin = ymin - 0.05*abs(ymax - ymin);
  ymax = ymax + 0.05*abs(ymax - ymin);
  hold on
  ...
  if (~isempty(Method.sampleDist))
    ...
    plot(t{i}( iSamples{i} ),rad2deg(wrapToPi(XOdoCal{i}( iSamples{i} ,3))),'x','Color',initialColorOrder(2,:))
    plot(t{i}( iSamples{i} ),rad2deg(wrapToPi(XGt{i}(     iSamples{i} ,3))),'x','Color',initialColorOrder(3,:))
  end
  grid on
  xlim([ 0    t{i}(end) ])
  ylim([ ymin ymax      ])
  ...
  legend('\theta_{unc}','\theta_{cal}','\theta_{gt}')

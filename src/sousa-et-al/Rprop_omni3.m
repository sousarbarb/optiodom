  ...
  if strcmp(rproptype,'rprop+')
    while (true)
      ...
      for i=1:Nsim
        ...
        for k=2:itf(i)
          % Common variables
          deltaD  = ( -D*Odo{i}(k,1) + D*Odo{i}(k,2) )*((3)^(1/2));
          deltaDN = ( -D*Odo{i}(k,1) - D*Odo{i}(k,2) + 2*D*Odo{i}(k,3) );
          deltaTH = ( -D*Odo{i}(k,1) - D*Odo{i}(k,2) -   D*Odo{i}(k,3) ) * a_pi_3nCeL;
          ...
          if ( abs(deltaTH)/0.040 < 1e-5 )
            ...
            derivParam(i,3) = derivParam(i,3) + ( -deltaD * a_cosTh + deltaDN * a_sinTh ) * ( XOdoF(i,3) - XGt{i}(1,3) );
          ...
          if (k == iSamples{i}(iseg))
            ...
            a_derivParam(3) = derivParam(i,3) * a_pi_3nCeL;

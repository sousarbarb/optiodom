function [HRob,ThRobFrame,imarkers] = computeRobotCenterOmni4(filenameCentre,filenameRob,TypeEulerAngle,visualize)
  ...
  % Confirm with the user the standar for the markers indexes
  im    = zeros(1,4);
  ...
  im(3) = input(prompt);
  prompt = 'Please insert the corresponding marker 4: ';
  im(4) = input(prompt);
  if (~visualize)
    close
  end
  % Compute the indexes of other markers
  imOther = zeros(1,countRigidBodyMarker-4);
  k=1;
  for i=1:countRigidBodyMarker
    if (i ~= im(1)) && (i ~= im(2)) && (i ~= im(3)) && (i ~= im(4))
      imOther(k) = i;
      k=k+1;
    end
  end
  ...
  for i=1:4                                     % Update the standard markers for the wheels shat markers
    MarkerAux(:,:,i) = Marker(:,:,im(i));
  end
  k=1;
  for i=5:countRigidBodyMarker                  % Update the other markers
  ...
  %% ROBOT LOGIC CENTRE
  robotlogiccentreaux1 = ( MarkerCG(:,1) + MarkerCG(:,2) ) / 2;
  robotlogiccentreaux2 = ( MarkerCG(:,3) + MarkerCG(:,4) ) / 2;
  robotlogiccentre     = ( robotlogiccentreaux1 + robotlogiccentreaux2 ) / 2;
  theta_robotworld     = atan2( robotlogiccentreaux1(1) - robotlogiccentre(1) , robotlogiccentreaux1(3) - robotlogiccentre(3) );
  ...
  if (visualize)
    ...
    for i=5:countRigidBodyMarker
      pointsRigidBodyOmni4 = [ pointsRigidBodyOmni4 ; rigidbody_omni_cg(3) rigidbody_omni_cg(1) ];
    ...
    plot([rigidbody_omni_cg(3)+0.05*cos(theta_y),rigidbody_omni_cg(3),rigidbody_omni_cg(3)-0.05*sin(theta_y)],...
        [rigidbody_omni_cg(1)+0.05*sin(theta_y),rigidbody_omni_cg(1),rigidbody_omni_cg(1)+0.05*cos(theta_y)],...
          'Color', [0 0 0])
    plot([MarkerCG(3,2),MarkerCG(3,1),MarkerCG(3,3),MarkerCG(3,4),MarkerCG(3,2)],...
        [MarkerCG(1,2),MarkerCG(1,1),MarkerCG(1,3),MarkerCG(1,4),MarkerCG(1,2)],...
          'Color', [150/255 150/255 150/255],'LineStyle','-')
    plot([MarkerCG(3,2),MarkerCG(3,3),MarkerCG(3,1),MarkerCG(3,4),MarkerCG(3,2)],...
        [MarkerCG(1,2),MarkerCG(1,3),MarkerCG(1,1),MarkerCG(1,4),MarkerCG(1,2)],...
          'Color', [150/255 150/255 150/255],'LineStyle','-')
    plot([MarkerCG(3,2),MarkerCG(3,1)],...
        [MarkerCG(1,2),MarkerCG(1,1)],...
          'Color', [150/255 150/255 150/255],'LineStyle','-')

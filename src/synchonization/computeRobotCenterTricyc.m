function [HRob,ThRobFrame,imarkers] = computeRobotCenterTricyc(filenameCentre,filenameRob,TypeEulerAngle,visualize)

  %% INITIALISATION
  strRigidBodyMarker = 'Rigid Body Marker';

  %% DATA PROCESSING OF FILE '..._CENTRE.CSV' <=> filenameCentre
  ...
  legend(legMarker)
  title('Original Markers of rigid body ROBOTDIFFCENTRE')
  ...
  imOther = zeros(1,countRigidBodyMarker-2);
  k=1;
  for i=1:countRigidBodyMarker
    if (i ~= im(1)) && (i ~= im(2))
      imOther(k) = i;
  ...
  for i=1:2                                     % Update the standard markers for the wheels shat markers
    MarkerAux(:,:,i) = Marker(:,:,im(i));
  end
  k=1;
  for i=3:countRigidBodyMarker                  % Update the other markers
  ...
  if (visualize)
    ...
    ylabel('y (m) \rightarrow')
    legend(legMarker)
    title('Modified Markers of rigid body ROBOTDIFFCENTRE')
  end
  
  %% DATA PROCESSING OF FILE '..._DIFF.csv' <=> filenameRob
  ...
  rigidbody_diff_cg = [ sum(BodyPos(:,1))/length(BodyPos(:,1)) , sum(BodyPos(:,2))/length(BodyPos(:,2)) , sum(BodyPos(:,3))/length(BodyPos(:,3)) ];
  ...
  % Compute the orientation of the rigid body ROBOTDIFF
  ...
  if (visualize)
    ...
    plot(rigidbody_diff_cg(3),rigidbody_diff_cg(1),'x')
    grid
    axis equal
    xlabel('x (m) \rightarrow')
    ylabel('y (m) \rightarrow')
  ...
  %% ROBOT LOGIC CENTRE
  robotlogiccentre = ( MarkerCG(:,1) + MarkerCG(:,2) ) / 2;
  theta_robotworld = atan2( MarkerCG(1,1)-robotlogiccentre(1) , MarkerCG(3,1)-robotlogiccentre(3) )+pi/2;
  ThRobFrame = theta_robotworld - theta_y;
  translation_vec  = [ robotlogiccentre(1)-rigidbody_diff_cg(1) , robotlogiccentre(2)-rigidbody_diff_cg(2) , robotlogiccentre(3)-rigidbody_diff_cg(3) ]';
  ...
  if (visualize)
    ...
    plot(rigidbody_diff_cg(3),rigidbody_diff_cg(1),'o')
    plot(robotlogiccentre(3),robotlogiccentre(1),'d')
    pointsRigidBodyOmni4 = [];
    for i=3:countRigidBodyMarker
      pointsRigidBodyOmni4 = [ pointsRigidBodyOmni4 ; rigidbody_diff_cg(3) rigidbody_diff_cg(1) ];
      pointsRigidBodyOmni4 = [ pointsRigidBodyOmni4 ; MarkerCG(3,i)        MarkerCG(1,i)        ];
    end
    plot(pointsRigidBodyOmni4(:,1),pointsRigidBodyOmni4(:,2),'Color', [150/255 150/255 150/255],'LineStyle','-')
    plot([rigidbody_diff_cg(3)+0.05*cos(theta_y),rigidbody_diff_cg(3),rigidbody_diff_cg(3)-0.05*sin(theta_y)],...
        [rigidbody_diff_cg(1)+0.05*sin(theta_y),rigidbody_diff_cg(1),rigidbody_diff_cg(1)+0.05*cos(theta_y)],...
          'Color', [0 0 0])
    plot([MarkerCG(3,1),MarkerCG(3,2)],...
        [MarkerCG(1,1),MarkerCG(1,2)],...
